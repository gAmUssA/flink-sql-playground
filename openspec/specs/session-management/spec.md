...
Idle session eviction SHALL be lazy, triggered on cache access, rather than polling on a fixed schedule. The eviction duration SHALL be determined by `FlinkProperties.sessionIdleTimeout()`. There SHALL be no `@Scheduled` cleanup method and no `@EnableScheduling` on the application class.
